...
bun_result_t bun_parse_header(BunParseContext *ctx, BunHeader *header) {
  ...
  validate_asset_table_size(header, file_size, asset_table_size);
  validate_string_table_size(header, file_size);
  validate_data_section_size(header, file_size);

  // Validate sections do not overlap each other
  validate_no_overlap(header, asset_table_size);
